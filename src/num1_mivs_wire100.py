...
---> Features:
-----> Wire100
-----> Supplier Overhead (+minimization, -similar to discount)
...
from src.utils.results_util import print_results_100, to_dict_100
...
async def main(_product,
    ...
    # _product = np.array(kwargs.get("_product"))
    # _req_qnty = np.array(kwargs.get("_req_qnty"))
    # _input_capital = np.array(kwargs.get("_input_capital"))
    # _cash = np.array(kwargs.get("_cash"))
    # _perech = np.array(kwargs.get("_perech"))
    # _supplier = np.array(kwargs.get("_supplier"))
    # _prices_title = np.array(kwargs.get("_prices_title"))
    ...
    for i in range(num_prod):  # per drug
        for j in range(num_sup):  # per supplier
            for k in range(num_perech):  # perech var
                var_wire_list[i, j, k] = model.NewIntVarFromDomain(
                    cp_model.Domain.FromIntervals([[0, 0], [_req_qnty[i], _req_qnty[i]]]),
                    fmt_perech % (i, j, k))
    ...
    # Adds an all-different constrai12. Select one price per prod of all perechs.
    ...
    # now = time()
    # print()
    # all_sum_per_prod = np.sum(var_wire_list, axis=1).sum(axis=1)
    ...
    solver.parameters.num_search_workers = int(kwargs.get("num_workers"))
    solver.parameters.max_time_in_seconds = int(kwargs.get("mivs_timeout"))
