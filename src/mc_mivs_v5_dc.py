"""
Multi-Constraint Multi-Item Vendor Selection ddouble combinatorics (Combination of Combinatorics)
"""
...
# def get_qnty_required(min_sum_per_supplier, wires, _input_capital):
#     _wires = np.where(_input_capital,  0, wires)
#     qnty = np.ceil(min_sum_per_supplier[:, None]/_wires)
#     qnty = np.max(qnty, axis=1).flatten()
...
async def main(_product,
    ...
    # _req_qnty_adj = get_qnty_required(_min_sum_per_supplier, _wires)
