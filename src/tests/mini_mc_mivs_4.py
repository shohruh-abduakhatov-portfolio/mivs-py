...
def SolveWithTimeLimitSampleSat():
    ...
    req_qnty = [10, 10, 10, 10]
    input_capital = 10000000
    ...
    per1_ = model.NewIntVar(0, req_qnty[0], 'per11')
    per12 = model.NewIntVar(0, req_qnty[0], 'per12')
    per13 = model.NewIntVar(0, req_qnty[0], 'per13')
    per14 = model.NewIntVar(0, req_qnty[0], 'per14')
    per2_ = model.NewIntVar(0, req_qnty[1], 'per21')
    per22 = model.NewIntVar(0, req_qnty[1], 'per22')
    per23 = model.NewIntVar(0, req_qnty[1], 'per23')
    per24 = model.NewIntVar(0, req_qnty[1], 'per24')

    per3_ = model.NewIntVar(0, req_qnty[2], 'per31')
    per32 = model.NewIntVar(0, req_qnty[2], 'per32')
    per33 = model.NewIntVar(0, req_qnty[2], 'per33')
    per34 = model.NewIntVar(0, req_qnty[2], 'per34')
    per4_ = model.NewIntVar(0, req_qnty[3], 'per41')
    per42 = model.NewIntVar(0, req_qnty[3], 'per42')
    per43 = model.NewIntVar(0, req_qnty[3], 'per43')
    per44 = model.NewIntVar(0, req_qnty[3], 'per44')
    ...
    model.Add(per1_ == req_qnty[0]).OnlyEnforceIf(per1_bool)
    model.Add(per1_ == 0).OnlyEnforceIf(per1_bool.Not())
    model.Add(per12 == req_qnty[0]).OnlyEnforceIf(per12_bool)
    model.Add(per12 == 0).OnlyEnforceIf(per12_bool.Not())
    model.Add(per13 == req_qnty[0]).OnlyEnforceIf(per13_bool)
    model.Add(per13 == 0).OnlyEnforceIf(per13_bool.Not())
    model.Add(per14 == req_qnty[0]).OnlyEnforceIf(per14_bool)
    ...
    model.Add(per22 == req_qnty[1]).OnlyEnforceIf(per22_bool)
    model.Add(per22 == 0).OnlyEnforceIf(per22_bool.Not())
    model.Add(per23 == req_qnty[1]).OnlyEnforceIf(per23_bool)
    model.Add(per23 == 0).OnlyEnforceIf(per23_bool.Not())
    model.Add(per24 == req_qnty[1]).OnlyEnforceIf(per24_bool)
    model.Add(per24 == 0).OnlyEnforceIf(per24_bool.Not())

    model.Add(per3_ == req_qnty[2]).OnlyEnforceIf(per3_bool)
    model.Add(per3_ == 0).OnlyEnforceIf(per3_bool.Not())
    model.Add(per32 == req_qnty[2]).OnlyEnforceIf(per32_bool)
    model.Add(per32 == 0).OnlyEnforceIf(per32_bool.Not())
    model.Add(per33 == req_qnty[2]).OnlyEnforceIf(per33_bool)
    model.Add(per33 == 0).OnlyEnforceIf(per33_bool.Not())
    model.Add(per34 == req_qnty[2]).OnlyEnforceIf(per34_bool)
    ...
    model.Add(per42 == req_qnty[3]).OnlyEnforceIf(per42_bool)
    model.Add(per42 == 0).OnlyEnforceIf(per42_bool.Not())
    model.Add(per43 == req_qnty[3]).OnlyEnforceIf(per43_bool)
    model.Add(per43 == 0).OnlyEnforceIf(per43_bool.Not())
    model.Add(per44 == req_qnty[3]).OnlyEnforceIf(per44_bool)
    ...
    s1 = per1_ + per12 + per13 + per14
    s2 = per2_ + per22 + per23 + per24
    s3 = per3_ + per32 + per33 + per34
    s4 = per4_ + per42 + per43 + per44

    model.Add(s1 >= 10).OnlyEnforceIf(b1)
    model.Add(s1 < 10).OnlyEnforceIf(b1.Not())
    model.Add(s2 >= 10).OnlyEnforceIf(b2)
    model.Add(s2 < 10).OnlyEnforceIf(b2.Not())
    model.Add(s3 >= 10).OnlyEnforceIf(b3)
    model.Add(s3 < 10).OnlyEnforceIf(b3.Not())
    model.Add(s4 >= 10).OnlyEnforceIf(b4)
    model.Add(s4 < 10).OnlyEnforceIf(b4.Not())

    model.Add(s1 == s1 - 100).OnlyEnforceIf(b1)
    model.Add(s1 == s1 + 0).OnlyEnforceIf(b1.Not())
    model.Add(s2 == s2 - 100).OnlyEnforceIf(b2)
    model.Add(s2 == s2 + 0).OnlyEnforceIf(b2.Not())
    model.Add(s3 == s3 - 100).OnlyEnforceIf(b3)
    model.Add(s3 == s3 + 0).OnlyEnforceIf(b3.Not())
    model.Add(s4 == s4 - 100).OnlyEnforceIf(b4)
    model.Add(s4 == s4 + 0).OnlyEnforceIf(b4.Not())
    ...
    qnty_diff3 = sum([per3_, per32, per33, per34]) - req_qnty[2]
    qnty_diff4 = sum([per4_, per42, per43, per44]) - req_qnty[3]
