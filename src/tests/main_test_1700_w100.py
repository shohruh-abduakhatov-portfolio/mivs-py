...
async def main():
    ...
    min_sum_per_supplier = (np.array([0]*len(supplier)) * decimal).astype(int).tolist()
    supp_overhead = (np.array([0]*len(supplier)) * decimal).astype(int).tolist()

    prices_title = ["cash_cost", ["pred_100", "pred_50", "pred_25"]]
    discount_threshold = [input_capital]*len(supplier)
    discount_percent = [0]*len(supplier)
